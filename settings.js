...
let markets = [
    ...
    // HIGH, 
    // LOW 
    // tBTCUSD, tLTCBTC, tETHBTC, tETCBTC, tRRTBTC, tZECBTC, tXMRBTC, tDSHBTC, tBTCEUR, tXRPBTC, tIOTBTC, tEOSBTC, tSANBTC, tOMGBTC, tBCHBTC, tNEOBTC, tETPBTC, tQTMBTC, tAVTBTC, tEDOBTC, tBTGBTC, tDATBTC, tQSHBTC, tYYWBTC, tGNTBTC, tSNTBTC, tBATBTC, tMNABTC, tFUNBTC, ttZRXBTC, tTNBBTC, tSPKBTC
    {
        marketName: 'bitfinex',
        ...
        last: function (data, coin_prices) { //Where to find the last price of coin in JSON data
            return new Promise(function (res, rej) {
                try {
                    for (var obj in data) {
                        if(data[obj][0].includes('BTC')) {
                            let cn = data[obj][0].replace("BTC", '');
...

    {
        ...
        last: function (data, coin_prices) { //Where to find the last price of coin in JSON data
            return new Promise(function (res, rej) {
                try {
                    for (var obj in data) {
                        if(data[obj]["symbol"].includes('BTC')) {
                            let coinName = data[obj]["symbol"].replace("BTC", '');
                            if (!coin_prices[coinName]) coin_prices[coinName] = {};
                            coin_prices[coinName].binance = data[obj]["price"];
                            //console.log(coin_prices["USD"]);
                        }
                    }
                    res(coin_prices);
